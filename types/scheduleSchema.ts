...
export const universityScheduleSchema = z.object({
    ...
    .refine(
        (data) => data.rescheduledDateList.length === data.rescheduledClassList.length,
        {
            message: "The length of rescheduledDateList and rescheduledClassList must be the same",
            path: ["rescheduledDateList", "rescheduledClassList"],
        }
    )
    .refine(
        (data) => data.deadlinesDate.length === 14,
        {
